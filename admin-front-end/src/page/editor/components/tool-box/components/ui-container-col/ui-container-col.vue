<template>
  <div class="ui-container-col left-bar-icon-1" :class="{disabled:disable}" @click="clickMe"></div>
</template>
...
<script>
  ...
  import { UDStage, UDUIContainerCol, UDUIContainer, UDPage } from '../../../../../../lib/ui-designer/index.js';
  export default {
    name: 'ui-container-col',
    props: [],
    props: ['name'],
    data() {
      return {};
    ...
    methods: {
      clickMe() {
        ...
        // 一个矩形
        let me = new UDUIContainerCol();
        me.name({ value: '列容器' + sliblingCount });

        me.w({ value: this.currentSelection.w().value });
        me.h({ value: 200 });
        me.bgColor({ value: '#f5f5ff' });
