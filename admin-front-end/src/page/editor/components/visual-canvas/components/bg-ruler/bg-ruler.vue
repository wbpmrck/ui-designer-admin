<template>
  <div id="bg-ruler">
    ...
    <div id="ruler-h" class="ruler" :style="{width:targetWidth+'px'}">
      <span
        class="mark"
        :style="{width:scaleUnit+'px'}"
        v-for="(mark,index) in horizontalMarks"
        :class="{'big':index %10 ===0,'medium':index %5 ===0,'small':index % 10 !== 0 && index % 10 !== 5}"
        :key="mark"
      >
    ...
    <div id="ruler-v" class="ruler" :style="{width:targetHeight+'px'}">
      <span
        class="mark"
        :style="{width:scaleUnit+'px'}"
        v-for="(mark,index) in verticalMarks"
        :class="{'big':index %10 ===0,'medium':index %5 ===0,'small':index % 10 !== 0 && index % 10 !== 5}"
        :key="mark"
      >
        <span class="label" v-if="index % 10 ===0">{{mark}}</span>
...
//TODO:本期的尺子功能，为了开发效率，其滚动功能是借助stage容器自己的滚动实现的。所以尺子本身会随着滚动而被隐藏。但实际使用过程中，可能希望水平
//方向的尺子不会在垂直方向上被滚动走。而垂直方向的尺子不会在水平方向上被滚走。这个以后再优化。
