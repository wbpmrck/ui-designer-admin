    ...
    <div class="item-wrapper">
      <div class="item-title" :class="{active:currentSelection===itemData}">
        ...
        <div
          ...
          :class="{'editor-hide':itemData.editorHide,'none':itemData.constructor.name ==='UDStage' || itemData.constructor.name ==='UDPage'}"
        >
          <span
            class="icon"
            v-show="!(itemData.constructor.name ==='UDStage' || itemData.constructor.name ==='UDPage')"
            @click="toggleHide"
            :title="itemData.editorHide?'编辑器内隐藏':'编辑器内可见'"
          ></span>
        </div>
        ...
        <div
          class="child-ctl"
          :class="{open:itemData.hasOwnProperty('__ud_attribute_children__') && itemData.fold,close:itemData.hasOwnProperty('__ud_attribute_children__') && !itemData.fold}"
        >
        ...
        <div :class="{['icon-'+itemType]:true,'icon-item':true}" @click="selectItem">
          ...
          <span class="icon-new-1" v-if="'UDUIContainerAbsolute'===itemType"></span>
        ...
        <div class="has-event">
          ...
          <span
            ...
            v-if="itemData.eventHandlers().value.length>0"
            :class="{active:currentScene===SCENE.EVENT_PANEL && currentSelection===itemData}"
          ></span>
        </div>
      </div>
...
<style lang="scss" scoped>
  ...
  .item-title {
    ...
    // 绝对定位容器图标(FIXME:)
    .icon-UDUIContainerAbsolute .icon-new-1 {
      width: 16px;
      height: 16px;
