...
class UDContainer extends UDObject {
    ...
    static getSupportActions() {
        return super.getSupportActions().concat([
            new UDAction({
                name: 'addChild',
                desc: '创建子对象(用于动态创建一个对象，并插入到自己children下面)',
                params: [
                    new UDAttribute({
                        name: 'child',
                        desc: '子对象',
                        valueType: UDObject.getType()
                    })
                ]
            }),
            new UDAction({
                ...
                params: [
                    new UDAttribute({
                        name: 'except',
                        desc: '排除以下子对象',
                        valueType: Types.ARRAY(UDObject.getType())
                    })
                ]
            }),
        ]);
    ...
    addChild(child) {
        if (!this.canAddChild(child.constructor.getType()) || this.indexOfChild(child) > -1) {
            ...
            child.parent = this;
            //add 2019年04月29日 ：对孩子进行属性启用设置
            child.enableAllPropsExcept(this.getDisableChildrenProps())
            return true;
        }
    }
}
